...
def fetch_listings():
    ...
    for link_el in soup.select("a[href*='/homes/98967']"):
        ...
        # MLS number from URL — needed for BOTH the card display AND
        # the image lookup below, so it's calculated first.
        mls_match = re.search(r"/(\d{7,9})$", href)
        mls_num   = mls_match.group(1) if mls_match else ""

        # ── Image (FIXED) ────────────────────────────────────────────────
        # Root cause: this listing's info (price/beds/sqft) and its photo
        # live in SIBLING columns, not one inside the other. The
        # "container" found above is just the narrow info-column —
        # climbing stopped as soon as it saw "Beds:"/"$", which happens
        # before reaching the shared ancestor that also holds the photo.
        # So container.select_one("img...") was searching a branch of
        # the DOM that never had an <img> in it.
        #
        # Fix: don't rely on DOM position at all. Every photo URL from
        # idxhome embeds this exact listing's MLS number
        # (e.g. .../images/MRD12729857/...), and we already have that
        # number from the URL. So search the WHOLE page for the image
        # tagged with this listing's MLS number — guaranteed correct
        # regardless of how nested/columned the layout is.
        img_el = None
        if mls_num:
            img_el = soup.select_one(f"img[src*='MRD{mls_num}']")

        # Fallback 1: narrower "mgrid.idxhome.com" search within container
        if not img_el:
            img_el = container.select_one("img[src*='mgrid.idxhome.com']")

        # Fallback 2: any <img> in container, last resort
        if not img_el:
        ...
        # Debug line — shows up in the GitHub Actions log for this step,
        # so you can see per-listing whether an image was found.
        print(f"  {street}: mls={mls_num or '—'}, img_src={'FOUND' if img_src else 'MISSING'}")
        # ────────────────────────────────────────────────────────────────
